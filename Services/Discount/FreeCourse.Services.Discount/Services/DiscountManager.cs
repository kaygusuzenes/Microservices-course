...
namespace FreeCourse.Services.Discount.Services
{
    public class DiscountManager : IDiscountService
    {
        ...
        {
            var deleteStatus = await _dbConnection.ExecuteAsync($"delete from discount whre id={id}");
        ...
        {
            var discount = (await _dbConnection.QueryAsync<Models.Discount>($"Select * from discount where userId={userId} and code={code}")).FirstOrDefault();
            if (discount is null)
                return Response<Models.Discount>.Fail("Discount not found", 404);
            return Response<Models.Discount>.Success(discount, 204);
